...
describe('format_ffmpeg_progress', function () {
    ...
    it('audio-rpad2', async function () {
        const items = await fs_read_json(fs_path_resolve(__dirname, 'progress.d/audio-rpad/stream_ffmpeg_progress.json'));
        items.forEach(v => v.expected_duration_us = 605877551);
        const actual = items.map(format_ffmpeg_progress);
        const expected = await fs_read_json(fs_path_resolve(__dirname, 'progress.d/audio-rpad/format_ffmpeg_progress2.json'));
        assert.deepStrictEqual(actual, expected);
    });
    ...
    it('video-shortest2', async function () {
        const items = await fs_read_json(fs_path_resolve(__dirname, 'progress.d/video-shortest/stream_ffmpeg_progress.json'));
        items.forEach(v => v.expected_duration_us = 5.88*1000000);
        const actual = items.map(format_ffmpeg_progress);
        const expected = await fs_read_json(fs_path_resolve(__dirname, 'progress.d/video-shortest/format_ffmpeg_progress2.json'));
        assert.deepStrictEqual(actual, expected);
    });
});
